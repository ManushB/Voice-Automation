...
def handle(text, mic, profile):
    ...
    try:
        results = graph.request("me/posts")
    ...
    if not len(results['data']):
        mic.say("You have no Facebook posts. ")
        return

    updates = []
    for notification in results['data']:
        if notification['id'] == "1074700095886597_1058348514188422":
        	updates.append(notification['message'])

    count = 1
    mic.say("You have " + str(count) +
            " recent Facebook posts. " + " ".join(updates) + ". ")
